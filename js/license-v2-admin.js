(function(){
  ...
  async function call(action,payload={}){const s=await session();if(!s)throw Object.assign(new Error("Silakan login sebagai developer."),{status:401});const timeoutMs=Number(cfg().requestTimeoutMs)||20000,controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);try{const res=await fetch(cfg().adminFunctionUrl,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${s.access_token}`,"apikey":cfg().supabasePublishableKey},body:JSON.stringify({action,...payload}),signal:controller.signal,cache:"no-store"});const data=await res.json().catch(()=>({ok:false,message:"Jawaban server tidak valid."}));if(!res.ok||data.ok===false){let message=data.message||`HTTP ${res.status}`;if(action==="sync_payment_status"&&/Action admin tidak dikenal/i.test(message)&&!data.admin_api_version)message="Edge Function ldm-license-admin-v2 yang aktif masih versi lama. Deploy ulang fungsi dari paket 27.8.1, lalu muat ulang Developer Center.";throw Object.assign(new Error(message),{status:res.status,data,serverVersion:data.admin_api_version||res.headers.get("X-LDM-Admin-Version")||null})}return data}catch(e){if(e.name==="AbortError")throw new Error(`Server admin tidak merespons dalam ${Math.round(timeoutMs/1000)} detik.`);throw e}finally{clearTimeout(timer)}}
