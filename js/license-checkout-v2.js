(function(){
    ...
    const LEGACY_KEYS=["ldmPublicCheckoutV261","ldmPublicCheckoutV26","ldmPublicCheckoutV25"];
    ...
    async function post(url,payload){
        ...
        try{
            ...
            if(!res.ok||data?.ok===false){const e=new Error(data?.message||`HTTP ${res.status}`);e.code=data?.code||"REQUEST_FAILED";throw e}
            return data;
    ...
    let currentPlan=null,currentReceipt=null;
    function renderSummary(){
        ...
        if(save){const s=savingFor(currentPlan.planCode,cycle);save.textContent=s>0?`Hemat ${rupiah(s)} dibanding ${cycle==="two_year"?"24":"12"}× pembayaran bulanan.`:"";save.hidden=s<=0}
    }
    function open(input){
        currentPlan={...input};
        const panel=el("publicCheckoutPanel"); if(!panel)return alert("Panel pembayaran belum tersedia.");
        ...
        renderSummary(); panel.hidden=false;panel.classList.add("open");panel.scrollIntoView({behavior:"smooth",block:"start"});
        setStatus("Isi data customer, lalu lanjutkan pembayaran melalui Lynk.id. Setelah webhook pembayaran sukses diverifikasi, lisensi aktif dan hasil tampil otomatis di halaman ini.","info");
    }
    function close(){const panel=el("publicCheckoutPanel");if(panel){panel.hidden=true;panel.classList.remove("open")}}

    function saveLast(data){localStorage.setItem(STORAGE_KEY,JSON.stringify({order_id:data.order_id,status_token:data.status_token,payment_gateway:"lynk",redirect_url:data.redirect_url||null,created_at:Date.now()}))}
    function readLast(){
        try{
            const cur=JSON.parse(localStorage.getItem(STORAGE_KEY)||"null");if(cur?.order_id&&cur?.status_token)return cur;
            for(const key of LEGACY_KEYS){const old=JSON.parse(localStorage.getItem(key)||"null");if(old?.order_id&&old?.status_token&&String(old.payment_gateway||"lynk").toLowerCase()==="lynk"){saveLast(old);return old}}
        }catch(_e){}
        return null;
    }
    ...
    function receiptText(r){return [
        "LOCDailyMar — DATA LISENSI",`Order ID: ${r.order_id||"-"}`,`Paket: ${r.plan_name||r.plan_code||"-"}`,
        `Periode: ${r.period_label||cycleLabel(r.billing_cycle)}`,`License Key: ${r.license_key||"-"}`,
        `Store Code: ${r.store_code||"-"}`,`Store UUID: ${r.store_id||"-"}`,`Network ID: ${r.network_id||"-"}`,
        `Email Owner: ${r.owner_email||"-"}`,`Masa berlaku: ${tanggal(r.expires_at)}`
    ].join("\n")}
    ...
    function renderReceipt(r){
        ...
        currentReceipt=r;const panel=el("licenseReceipt");if(!panel)return;
        setText("receiptOrderId",r.order_id);setText("receiptPlan",`${r.plan_name||r.plan_code||"-"} · ${r.period_label||cycleLabel(r.billing_cycle)}`);
        setText("receiptLicenseKey",r.license_key);setText("receiptStoreCode",r.store_code);setText("receiptStoreId",r.store_id);setText("receiptNetworkId",r.network_id);setText("receiptOwnerEmail",r.owner_email);setText("receiptExpires",tanggal(r.expires_at));
        ...
        const provision=el("receiptProvisionNote");if(provision){provision.textContent=r.provision_status==="ready"?"Lisensi dan akun Owner sudah siap digunakan.":`Lisensi aktif, tetapi provisioning akun belum selesai${r.provision_error?`: ${r.provision_error}`:"."}`;provision.className="receipt-provision "+(r.provision_status==="ready"?"ok":"warn")}
    ...
    async function status(orderId,statusToken,quiet=false){
        ...
        if(!quiet){
            if(data.payment_status==="paid")setStatus("✅ Pembayaran Lynk.id sudah terverifikasi. Data lisensi tersedia di bawah.","success");
            else if(["failed","expired","cancelled"].includes(String(data.payment_status||"").toLowerCase()))setStatus(`Pembayaran berstatus ${data.payment_status}. Hubungi Support bila dana sudah terpotong.`,"error");
    ...
    function validLynkUrl(raw){try{const u=new URL(String(raw||"").trim());return u.protocol==="https:"&&(u.hostname.toLowerCase()==="lynk.id"||u.hostname.toLowerCase()==="www.lynk.id")?u.href:""}catch{return ""}}
    function resolveLynkUrl(planCode,cycle){const raw=String(cfg().links?.[planCode]?.[cycle]||"").trim();const url=validLynkUrl(raw);if(!url)throw new Error(`Link Lynk.id untuk ${currentPlan?.planName||planCode} · ${cycleLabel(cycle)} belum dikonfigurasi di js/license-v2-config.js.`);return url}
    function validate(){
    ...
    async function submitLynk(){
        validate();const cycle=el("checkoutPeriod").value,url=resolveLynkUrl(currentPlan.planCode,cycle),button=el("checkoutPayBtn");
        button.disabled=true;button.textContent="Membuat order Lynk.id…";
        try{
            setStatus("Membuat pending order di License Authority…","info");
            const data=await callOrder({plan_code:currentPlan.planCode,billing_cycle:cycle,customer_name:el("checkoutName").value.trim(),customer_email:el("checkoutEmail").value.trim(),customer_phone:el("checkoutPhone").value.trim(),store_name:el("checkoutStoreName").value.trim(),store_code:el("checkoutStoreCode").value.trim().toUpperCase(),checkout_url:url});
            saveLast(data);savePending({order_id:data.order_id,status_token:data.status_token,plan_code:currentPlan.planCode,billing_cycle:cycle,amount:data.amount,redirect_url:url});
            const check=el("checkoutCheckBtn");if(check){check.hidden=false;check.disabled=false}
            setStatus(`✅ Order ${data.order_id} dibuat. Lynk.id dibuka di tab baru. Gunakan email yang sama dengan Email Owner agar webhook dapat mencocokkan transaksi.`,"success");
    ...
    async function checkLast(){const last=readLast();if(!last?.order_id||!last?.status_token)throw new Error("Belum ada order Lynk.id pada perangkat ini.");return status(last.order_id,last.status_token,false)}
    async function loadRefundPolicy(){const days=el("refundPolicyDays"),state=el("refundPolicyState");if(!days&&!state)return;try{const d=await callStatus({action:"refund_policy"});const p=d.policy||{},n=Math.max(1,Number(p.refund_window_days||3));if(days)days.textContent=`${n} hari kalender`;if(state)state.textContent=p.enabled===false?"Refund sedang dinonaktifkan sementara.":`Kebijakan aktif · versi ${p.policy_version||"-"}.`}catch(_e){if(days)days.textContent="3 hari kalender";if(state)state.textContent="Kebijakan dinamis belum dapat dimuat."}}
    function init(){
        ...
        el("checkoutPeriod")?.addEventListener("change",renderSummary);el("checkoutCloseBtn")?.addEventListener("click",close);
        ...
        loadRefundPolicy();
        const params=new URLSearchParams(location.search),last=readLast();if(params.get("payment")==="return"&&last?.order_id)setTimeout(()=>status(last.order_id,last.status_token,false).catch(()=>{}),700);
    }
    window.LDMCheckoutV2=Object.freeze({open,close,checkLast,init});
