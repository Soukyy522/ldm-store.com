-- ============================================================================
-- LocDailyMar 25.3.1
-- Owner Cabang: harga beli tersembunyi, subtotal/total terlihat, harga terkunci.
-- Jalankan pada Supabase SQL Editor PROJECT APLIKASI.
-- Aman dijalankan setelah 25.2 atau 25.3.
-- ============================================================================
...
begin
    ...
    if to_regclass('public.purchase_orders') is null
       ...
       or to_regclass('public.goods_receipt_items') is null then
        raise exception 'Tabel procurement Tahap 11 belum lengkap.';
    end if;
    if to_regprocedure('public.ldm_current_store_id()') is null
       or to_regprocedure('public.ldm_current_role()') is null
       or to_regprocedure('public.ldm_is_primary_owner()') is null then
        raise exception 'Fungsi konteks cloud/Owner Pusat belum tersedia.';
    end if;
...
-- Endpoint internal untuk kalkulasi halaman PO/GR.
-- UI menyembunyikan harga dari Owner Cabang dan server tetap menolak perubahan.
...
-- Owner Cabang menerima subtotal/total, tetapi tidak menerima harga satuan.
-- Admin tetap menerima nilai procurement 0 seperti aturan sebelumnya.
...
as $$
    select jsonb_build_object(
        'purchase_orders',coalesce((
            select jsonb_agg(
                to_jsonb(po) || jsonb_build_object(
                    'total_value',case
                        when public.ldm_current_role()='owner' then coalesce(po.total_value,0)
                        else 0
                    end
        ...
        'purchase_order_items',coalesce((
            select jsonb_agg(
                to_jsonb(i) || jsonb_build_object(
                    'purchase_price',case when public.ldm_is_primary_owner() then coalesce(i.purchase_price,0) else 0 end,
                    'package_purchase_price',case when public.ldm_is_primary_owner() then coalesce(i.package_purchase_price,0) else 0 end,
        ...
        'goods_receipts',coalesce((
            select jsonb_agg(
                to_jsonb(gr) || jsonb_build_object(
                    'total_value',case
                        when public.ldm_current_role()='owner' then coalesce(gr.total_value,0)
                        else 0
                    end
        ...
        'goods_receipt_items',coalesce((
            select jsonb_agg(
                to_jsonb(i) || jsonb_build_object(
                    'purchase_price_before',case when public.ldm_is_primary_owner() then coalesce(i.purchase_price_before,0) else 0 end,
                    'purchase_price',case when public.ldm_is_primary_owner() then coalesce(i.purchase_price,0) else 0 end,
                    'package_purchase_price',case when public.ldm_is_primary_owner() then coalesce(i.package_purchase_price,0) else 0 end,
                    'line_subtotal',case when public.ldm_current_role()='owner' then coalesce(i.line_subtotal,0) else 0 end
                ) order by i.created_at
...
-- Hanya Owner Pusat boleh menentukan harga dari browser.
-- Owner Cabang dan Admin selalu dipaksa memakai harga tepercaya dari server:
-- harga PO bila GR terkait PO, atau harga Master Barang pada toko aktif.
...
values ('branch_owner_procurement_policy','25.3.1-totals-only')
...
-- Hasil yang benar: ketiga kolom true dan value 25.3.1-totals-only.
select
    ...
    has_function_privilege(
        ...
        'EXECUTE'
    ) as procurement_endpoint_execute_ok;

select key,value,updated_at
from public.ldm_system_meta
where key='branch_owner_procurement_policy';
